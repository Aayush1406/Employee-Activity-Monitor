...
class ActivityMonitor:
    ...
    def get_foreground_process_id(self):
        ...
        handle = self.user32.GetForegroundWindow() # retrieves handle (unique indentifier of a system resource) of the window with which user is currentlt working.            
        pid = wintypes.DWORD() # create a DWORD data type created using python ctypes. Its a 32 bit unsigned integer            
        self.user32.GetWindowThreadProcessId(handle, ctypes.byref(pid)) #GetWindowThreadProcessID(), returns the identifier of the thread that created the window. Eg: Process ID.
    ...
    def get_title_bar_info(self,handle):
        titleBarInfo = Titlebarinfo()
        activity_monitor = ActivityMonitor()
        titleBarInfo.cbSize = ctypes.sizeof(Titlebarinfo)
        activity_monitor.user32.GetTitleBarInfo(handle,ctypes.byref(titleBarInfo))    
        return titleBarInfo

    def get_executable_path_from_Pid(self,pid):
        try:
    ...
    def print_foreground_details(self,foreground_details):
    ...
    def get_foreground_process_details(self,pid, current_handle):

        
        executable_path = self.get_executable_path_from_Pid(pid)
        child_name = self.get_uwp_app_name(pid)
        title_bar_info = self.get_title_bar_info(current_handle)
        window_title = self.get_window_title(current_handle,pid)
        process_create_time = self.get_process_create_time(pid)                
    ...
    def insert_foreground_process_details_in_db(self,foreground_details):
        try:
    ...
    def calculate_window_end_time(self,old_pid):
    ...
    def calculate_window_start_time(self, pid, window_title_name):
    ...
    def set_active_window_status(self, pid):
    ...
    def set_inactive_window_status(self, pid):
    ...
    def check_process_status(self, pid,handle):
        process_name = self.get_process_name(pid)
        ...
        try:
            ...
            if record_value and self.get_process_name(pid)=="No Such Process":   
    ...
    def calculate_process_duration(self, old_pid):
        ...
        try:
            ...
            if rows:
                ...
                conn.commit()
            self.set_inactive_window_status(old_pid)
    ...
    def keep_running_script(self,time_interval):
        ...
        while True:
            ...
            current_pid, current_handle = self.get_foreground_process_id()
        
            if old_pid != current_pid and old_handle != current_handle and current_pid!= 22824:
                foreground_details = self.get_foreground_process_details(current_pid, current_handle)
                self.print_foreground_details(foreground_details)
                self.insert_foreground_process_details_in_db(foreground_details)                                   
                self.calculate_window_start_time(current_pid, foreground_details['window_title'])
    ...
    def main(self):
        ...
        time_interval = 1
        self.keep_running_script(time_interval)
